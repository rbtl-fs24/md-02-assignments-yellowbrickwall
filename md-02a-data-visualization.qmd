...
4.  The plot is identical to the plot created with `geom_col()`. Why? What does the `geom_bar()` function do? Write your text here:
    ...
    -   Column charts require set values for both axes. It appears that geom_bar automatically displays the count on the y axis. It therefore displays the amount of data that fit a certain category, instead of showing specific values.
...
6.  Describe how the `geom_histogram()` function is similar to the `geom_bar()` function.
    ...
    -   It also does not require a set y-axis value and represents the quantity of data points that meet certain criteria.
...
7.  What happens by adding the 'breaks' argument? Play around with the numbers inside of `seq()` to see what changes. Describe here what you observe:
    ...
    -   The first value is the lower border, the second value is the upper border, and the third value is the histogram spacing withing this range.
...
4.  What is stored in the `country_colors` object? Find out by executing the object in the Console (type it to the Console and hit enter). Do the same again, but with a question mark `?country_colors`.
    ...
    -   It contains two vectors which store color values for the continents and countries. The country colors are slightly adjusted versions of their respective continent color.
